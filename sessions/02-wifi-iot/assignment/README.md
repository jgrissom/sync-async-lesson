...
Finish what Part C started: **your** Session 1 reaction game, fully online. Every round reports to the class scoreboard, standings print between rounds, and the game shrugs off network failures. Work at your Session 1 bench pairing.

## What the finished game does

1. On startup it connects to Wi-Fi (clear success message with its IP) — then plays exactly like Session 1: blinking red wait beacon, green GO, cap-color win flashes, false-start detection.
...
Lost your Session 1 game? Tell the instructor — don't rebuild it from scratch on the clock.

## Stretch goals (extra credit)

- **Reaction-time telemetry:** measure each win's reaction time with `time.ticks_diff()` (Session 1 stretch) and include it in the POST payload as `"ms"`. (The server ignores unknown fields today — telemetry that's ready before the backend is happens in real teams too.)
